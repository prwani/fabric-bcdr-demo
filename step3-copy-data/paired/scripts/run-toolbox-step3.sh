...
Print the exact Step 3 paired-region assets to run from prwani/fabric-toolbox.
EOF
}
...
COPY_WAREHOUSE_README="${TOOLBOX_ROOT}/tools/copy-warehouse/README.md"

if [[ "$CHECK_LOCAL" == true ]]; then
  check_file "$DR_NOTEBOOK"
  check_file "$UTILS_NOTEBOOK"
  check_file "$INGEST_SQL"
  check_file "$COPY_WAREHOUSE_README"
fi

cat <<EOF
Step 3 paired-region toolbox references
======================================
...
Warehouse copy helper:
  Local: ${COPY_WAREHOUSE_README}
  URL:   ${TOOLBOX_REPO_URL}/blob/main/tools/copy-warehouse/README.md
EOF
